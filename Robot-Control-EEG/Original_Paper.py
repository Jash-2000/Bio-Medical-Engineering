...
def RNN(X, weights, biases):
    ...
    X_hidd4 = tf.reshape(X_hidd3, [-1, batch_size, 64])# (step, batch_size, n_hidden)

    # cell
    ##########################################

    # basic LSTM Cell.
    lstm_cell_1 = tf.nn.rnn_cell.BasicLSTMCell(64, forget_bias=1.0, state_is_tuple=True)
    lstm_cell_2 = tf.nn.rnn_cell.BasicLSTMCell(64, forget_bias=1.0, state_is_tuple=True)
...
weights = {
    # (28, 128)
    'in': tf.Variable(tf.random_normal([n_inputs, 64]),name="weights_in"),
    #(128,128)
    'hidd2': tf.Variable(tf.random_normal([64, 64]), name="weights_hidd2"),
    'hidd3': tf.Variable(tf.random_normal([64, 64]), name="weights_hidd3"),
    'hidd4': tf.Variable(tf.random_normal([64, 64]), name="weights_hidd4"),
    # (128, 10)
    'out': tf.Variable(tf.random_normal([64, n_classes]), name="weights_out"),
}

biases = {
    # (128, )
    'in': tf.Variable(tf.constant(0.1, shape=[64]),trainable=True,name="biases_in"),
    #(128,)
    'hidd2': tf.Variable(tf.constant(0.1, shape=[64 ]), trainable=True,name="biases_hidd2"),
    'hidd3': tf.Variable(tf.constant(0.1, shape=[64]), trainable=True,name="biases_hidd3"),
    'hidd4': tf.Variable(tf.constant(0.1, shape=[64]), trainable=True,name="biases_hidd4"),
    # (10, )
...
with tf.Session() as sess:
    ...
    while step < 1500:# 1500 iterations
        ...
        if sess.run(accuracy, feed_dict={x: b,y: label_testing,})>0.945:
            print(
            "The lamda is :", lamena, ", Learning rate:", 0.005, ", The step is:", step, ", The accuracy is: ",
        ...
        if step % 10 == 0:
            ...
            print("The lamda is :",lamena,", Learning rate:",0.005,", The step is:",step,", The accuracy is:", hh)
    ...
    save_path = saver.save(sess, "generalmodel2/eeg_rawdata_runn_model" + str(lamena) + str(
        0.005) +str(nodes)+str(n_group)+ ".ckpt")
